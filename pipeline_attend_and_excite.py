...
def bhattacharyya(mu_1, mu_2, sigma_1, sigma_2, shape):
    ...
    for i in range(shape[0]):
        for j in range(shape[1]):
            ...
            p = torch.exp(-((i - mu_1[0])**2 + (j - mu_1[1])**2)/(2*sigma_1**2))
            q = torch.exp(-((i - mu_2[0])**2 + (j - mu_2[1])**2)/(2*sigma_2**2))

            metric = metric + p*q #torch.sqrt(p*q)

    metric = metric/(shape[0]*shape[1])

    return metric


def spread(att_map, peak):

    factor = 1/att_map[peak[0], peak[1]]

    spread = 0 
    for i in range(att_map.shape[0]):
        for j in range(att_map.shape[1]):
            spread += np.sqrt((i - peak[0])**2 + (j - peak[1])**2)*att_map[i, j]*factor
    return spread
...
class AttendAndExcitePipeline(StableDiffusionPipeline):
    ...
    def _aggregate_attention_per_token(self, attention_store: AttentionStore,
                                       indices_to_alter: List[int],
                                       attention_res: int = 16):
        """ Aggregates the attention for each token. """
        attention_maps = aggregate_attention(
            attention_store=attention_store,
            res=attention_res,
            from_where=("up", "down", "mid"),
        ...
        # Softmax over all attention maps but the first (<SOS> token)
        last_idx = -1
        prompt = self.prompt
        if isinstance(self.prompt, list):
            prompt = self.prompt[0]
        ...
        attention_maps = torch.permute(attention_for_text[:, :, indices_to_alter], (2, 0, 1))
    ...
    @staticmethod
    def _compute_loss_mine(attention_map_per_index: torch.Tensor, return_losses: bool = False) -> torch.Tensor:
        raise NotImplementedError("Add your favorite distance loss here!")
        # smoothing = AverageSmoothing(channels=1, kernel_size=3, dim=2).cuda()
        # smooth_maps = torch.squeeze(smoothing(attention_map_per_index.unsqueeze(1)))
    ...
    def _perform_iterative_refinement_step(self,
        ...
        while loss > target_loss:
            ...
            loss, losses = self._compute_loss_mine(max_attention_per_index, return_losses=True)
...
# https://stackoverflow.com/questions/34958855/sublimetext3-fold-unfold-all-methods
    ...
    def _perform_iterative_refinement_step_mine(self,
                                           latents: torch.Tensor,
                                           indices_to_alter: List[int],
                                           threshold: float,
                                           text_embeddings: torch.Tensor,
        ...
        while True:
            ...
            # Get max activation value for each subject token
            attention_map_per_index = self._aggregate_attention_per_token(
                attention_store=attention_store,
                indices_to_alter=indices_to_alter,
                attention_res=attention_res)
            
            breakpoint()
            loss, losses = self._compute_loss_mine(attention_map_per_index, return_losses=True)

            print (iteration, loss, target_loss)

            if (loss <= target_loss or iteration > 100):
                break
            iteration += 1
        ...
        loss, losses = self._compute_loss_mine(attention_map_per_index, return_losses=True)
        print(f"\t Finished with loss of: {loss}")
        return loss, latents, attention_map_per_index
            ...
            for i, t in enumerate(timesteps):
                ...
                with torch.enable_grad():
                    ...
                    if not run_standard_sd:
                        ...
                        # If this is an iterative refinement step, 
                        if i in thresholds.keys():
                            del noise_pred_text
                            torch.cuda.empty_cache()
                            loss, latents, attention_map_per_index = self._perform_iterative_refinement_step_mine(
                                latents=latents,
                                indices_to_alter=indices_to_alter,
                        ...
                        if i < max_iter_to_alter:
                            loss = self._compute_loss_mine(attention_map_per_index=attention_map_per_index)
                            # if loss != 0:
